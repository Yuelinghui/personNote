...
将元素嵌入`<div>`就是指示所有zh这些元素属于同一个组。可以使用id属性为`<div>`提供一个唯一的标签

`<div>`就相当于一个容器，可以把元素一同放在这个容器里。它不仅相当于逻辑容器，还可以相当于图形容器（对`<div>`指定样式和定位时）

**在页面中要使用`<div>`，但不要滥用。若这样做有助于将页面分解为逻辑区，从而保证结构清晰并便于指定样式，那么可以增加更多的结构。若只是为了在页面中创建大量结构而增加`<div>`，就只会让页面复杂，而没有任何实际好处**

text-align会对块元素中的所有内联内容对齐。text-align属性只能在块元素上设置，若直接在内联元素（如`<img>`）上使用，则不起作用

我们只想选择某些元素的子孙元素，选择器的写法为：
...
```
只有当<h2>是一个id为“xxx”的元素的直接孩子时，才会选择它
...
`<div>`和`<span>`没有默认样式，所以若不设置样式，看不出来变化

###根据元素状态指定样式

`<a>`元素有多种状态：未访问，已访问或悬停等，每种状态时链接文本展示的效果都不一样
...
**伪类**，看起来是一类，但它不是真正的类。我们可以对伪类指定样式，但没有人在HTML中真正输入这些伪类

##布局与定位

使用多栏布局的成熟，专业的设计。让HTML元素动起来，创建一些有真正布局的页面

###使用流

流实际上就是浏览器在页面上摆放HTML元素所用的方法。浏览器从HTML文件最上面开始，从上到下沿着元素流逐个显示所遇到的各个元素

内联元素在水平方向上会互相挨着，总体上会从左上方流向右下方

* 浏览器并排放置两个内联元素时，外边距会相加
* 浏览器上下放置两个块元素时，它们共同的外边距会折叠在一起，折叠的外边距高度就是最大的外边距高度
* 若一个元素嵌套在另一个元素中，它们都有外边距，就会折叠。只要两个垂直外边距碰到一起，它们就会折叠。若外面的元素有一个边框，那么两个元素的外边距就不会碰到一起，这样就不会折叠，若把边框去掉，这两个外边距就会折叠

使用float来是块元素浮动。对内联元素定位时，它们会考虑浮动元素的边界，因此会围绕着浮动元素

###流体与冻结设计

* 流体布局：不论将浏览器调整到多大的宽度，布局都会扩展，填满整个浏览器，使用户能够充分利用他们的屏幕空间
* 冻结布局：让布局锁定，当用户调整屏幕大小时，设计仍能保持原样。元素冻结在页面上，元素根本不能移动

冻结布局就是把宽度设置成固定值

流体布局和冻结布局中间的状态是凝胶布局：锁定页面中内容区的宽度，不过会将它在浏览器中居中

###定位

绝对定位：position：absolute；然后设置top，right等属性

绝对定位于浮动元素有所不同，因为流中的元素会调整它们的内容来适应浮动元素的边界，绝对定位元素对其他元素没有任何影响

绝对定位可以分层放置，一个元素可以放在另一个绝对定位元素上面。使用z-index属性（上面的元素z-index更大）

position的默认值是static（静态）
一个元素绝对定位时，会把它从页面的正常流中删除

position属性有4个值：static，absolute，fixed和relative

fixed（固定）定位将元素放在相对于浏览器窗口的一个位置上（而不是相对于页面），固定元素永远不会移动，滚动页面也不会移动，调整窗口大小也不会移动

relative(相对)定位会让元素正常流入页面，不过在页面上显示之前要进行偏移

##现代HTML

研究了人们如何使用`<div>`元素，然后增加了一些新的元素来表示这些结构

利用H5可以把原来的`<div>`换成一些更特定的元素，能够更明确地指示其中包含什么类型的内容

`<time>`显示时间。它有一个重要的属性：datetime。

如果元素内容没有采用官方的格式来写，就必须有datatime属性

```
<time datetime="2016-12-29">12/29/2016</time>
```

###播放视频

`<video>`元素很像`<img>`元素，但是它不是void元素，它有开始和结束标记

```
<video controls autoplay width="512" height="288" src="video/XXX.mp4"></viedo>
```

`poster="img/poster.png"`给视频提供一个可选的海报图像，视频未播放时就会显示这个图像

`preload`用于细粒度地控制视频如何加载，来实现优化。大多数情况下，浏览器会根据是否设置autoplay以及用户的带宽来选择加载多少视频。设置为“none”（在用户真正“播放”视频之前不下载视频），也可以设置为“metadata”（下载视频元数据，但不下载视频内容），或设置“auto”，让浏览器来做决定

`loop`是一个布尔属性，决定视频结束播放之后是否会自动重新开始播放

**HTML5规范允许采用任何视频格式，具体支持哪些格式由浏览器实现来确定**

在`<video>`元素中可以对应每种格式分别使用一个`<source>`元素来提供一组视频，每个视频分别有自己的格式，可以让浏览器选择它支持的第一种格式（不要与src属性混淆）

H5的新元素

* `<aside>`：表示放在主内容旁边的内容，比如边栏或引用
* `<mark>`：用于突出显示某些文本，就像记号笔一样
* `<audio>`：用它在页面中包含声音内容
* `<time>`：显示一个时间，一个日期或者一个日期时间
* `<progress>`：显示任务的完成进度
* `<footer>`：定义一个区块的底部或整个文档的页脚
* `<meter>`：显示某个范围的度量，比如一个温度计
* `<article>`：用来标记类似新闻报道或博客帖子等独立的内容
* `<canvas>`：用来在页面中显示用JavaScript绘制的图像和动画
* `<section>`：使用这个元素定义文档的主要区块
* `<header>`：有首部的区块和整个文档的页面
* `<video>`：在页面中加入一个视频
* `<nav>`：用于导航的所有链接组织在一起
* `<figure>`：用来定义类似图片，图表甚至代码清单等独立的内容

##建立表格

HTML有一个`<table>`元素专门负责标记表格数据

* `<tr></tr>`表示行
* `<th></th>`表示每一列的表头
* `<td></td>`表示单元格

**表格提供了一种在HTML中指定表格数据的方法，表格由行中的数据单元格组成，列隐含地定义在行中。一般来说，表格不用来提供表现，那是CSS的工作**

`<caption></caption>`可以用来指定表格的标题

在表格中，单元格之间的空间叫做：border-spacing（边框间距），所以在表格中不用外边距。不能单独地设置各个表格单元格的“外边距”，而要为所有单元格设置一个共同的间距

`border-collapse`的CSS属性可以折叠边框，使单元格之间根本没有边框间距

nth-child这个伪类可以选择嵌套在一个元素中的奇数和偶数项。可以使用数字n来指定简单的表达式，从而在选择元素时有更多方式

###单元格跨多行

使用rowspan属性指定一个表格数据单元格占多少行，然后从单元格所跨越的其他行中删除相应的表格数据元素

```
<td rowspan="2">XXXX</td>
```

使用colspan属性指定一个表格数据单元格占多少列

###创建列表标记

list-style-type属性，可以控制列表中使用的项目符号（列表标记）
* disc：默认的列表标记类型
* circle：简单的圆形标记
* square：方块标记
* none：删除所有列表标记

list-style-image允许为列表设值标记图像
list-style-position设置为“inside”，文本就会在标记下回绕。设置成“outside”，就只在文本下回绕

##实现交互

表单实际上就是一个包含输入域的Web页面，允许你输入信息。提交表单时，这些信息会打包并发送到一个Web服务器，由一个服务器脚本处理

使用`<form>`元素创建表单

action属性包含Web服务器的URL，method属性确定表单数据如何发送到服务器
