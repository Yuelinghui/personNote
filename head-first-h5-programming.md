...
* GPS：利用卫星定位
* IP地址：基于IP地址的位置信息使用一个外部数据库将IP地址映射到一个物理位置。这种方法在城市级很可靠
* 蜂窝电话：蜂窝电话三角定位可以根据你与一个或多个蜂窝电话基站的距离来确定位置。若位置很偏僻，位置的精度可能就不容乐观了
* Wifi：使用一个或多个wifi接入点完成三角定位

没有办法知道你的设备使用了哪种方法。可能先使用蜂窝电话三角定位，得到一个大致的位置，然后再用wifi或GPS提供一个更精确的位置。
